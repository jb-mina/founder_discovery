# Founder Discovery — Claude Code Memory
...
- 에이전트 구현 현황: `docs/agents-inventory.md`
...
4단계 루프를 따라, 자신과 맞는 문제를 찾고 검증하도록 돕는 Discovery Copilot.

"아이디어 추천기"가 아닙니다. "의사결정 보조 장치"입니다.
...
- **아이디어를 생성하지 않는다.** 에이전트는 사용자의 생각을 구조화하고 비교·검증을 돕는다.
  Validation Designer가 만드는 것은 "답"이 아니라 "검증 대상인 솔루션 가설"이다.
...
- **속도보다 납득이다.** 북극성 지표는 "이 문제를 정말 풀어도 되겠다는 확신"이다.
...
- **Fit Judge Agent** — 큐레이터. 추천 시 Self map의 특정 엔트리 ID를 근거로 인용한다.
  점수는 사용자만 매긴다.
- **Validation Designer Agent** — 실행 설계자. 2주 기본, 최대 4주 안에 실행 가능한 스텝만.
...
**트리거** — Claude Code와 플래닝 후 방향이 정해진 시점, 또는 Known Deviations에 새 항목이 추가된 시점.


## 10. Known Deviations — 원칙과 현실의 괴리

> 근거: `docs/agents-inventory.md` (2026-04-22).
> 인벤토리를 다시 돌리면 이 섹션도 함께 갱신할 것.

**코드 정상화 대상** (원칙은 유지, 코드를 원칙 쪽으로 이동)

1. **Zod 스키마 부재** — Self Insight 완료 (2026-04-21: `/api/self-insight/extract` Zod 도입, 마커 파싱 제거). 나머지 4개(Problem Scout, Validation Designer, Fit Judge, Reality Check)는 점진 마이그레이션 대기.
2. **`lib/agents/` 3파일 구조 부재** — 전원 `route.ts` 인라인. 프롬프트 메이저 튜닝 시점에 이동.
3. **`lib/db/` 추출 부재** — Prisma 쿼리가 각 route에 인라인. `lib/db/` 레이어로 점진 추출.
4. **Reality Check 4개 페르소나 단일 파일** — `api/reality-check/route.ts` 안에 공존. `lib/agents/` 이동 시 함께 분리.
5. **시드 하드코딩** — `api/problems/seed/route.ts` → `seeds/problem-cards.json` 이동 예정.
6. **Fit Judge 근거 인용 부재** — `reason` 필드가 Self map 엔트리 ID 참조가 아닌 자체 설명. 스키마 확장 필요.
7. **Reality Check 반증 질문 누락** — coldInvestor/honestFriend 프롬프트에 "비판 시 반증 질문 또는 다음 액션 동반" 추가 필요.
8. **Validation Designer `weeklySteps` 4주 고정** — 기본 2주, 최대 4주로 수정.
9. **Problem Scout URL hallucination 리스크** — YC/Sequoia/a16z 베이스 URL 프롬프트 주입은 완화책. 저장 전 URL 유효성 검증 또는 web_fetch 도구 도입 필요.

**미구현 제품 기능** (부채가 아니라 로드맵)

10. **`LearningLog` 엔티티** — 현재 `ValidationPlan.learnings` 문자열 누적. 별도 엔티티 분리.
11. **LearningLog → SelfMap/ProblemCard 역류** — 제품 정체성 기능. 최우선.
12. **Problem Scout 주기 갱신 job**.
13. **Weekly Digest**.

**확정된 스키마 추가·변경 대기**

14. **`ValidationPlan.ideaDraft` → `solutionHypothesis` 리네이밍** — 옵션 (b) 채택 확정.
    Prisma 마이그레이션 + 프롬프트 문구 + UI 카피 세트로 한 번에.
15. **`ProblemCard.sourceVerifiedAt` 추가** — URL 검증 타임스탬프.
16. **`ValidationPlan.timeboxWeeks` 추가** — 기본 2, 최대 4.
17. **`SelfMap` 엔트리에 `sourceMessageId` 추가** — 추출 출처 추적.

**명시적 비결정 사항** (현재 상태 유지)

- 필드명 스타일 변경(`attraction → pull` 등) — **하지 않음.** 마이그레이션 비용 대비 가치 없음.
- 패키지 매니저 — **`npm` 유지.** pnpm 전환 계획 없음.
- `docs/agent-principles.md` — **만들지 않음.** 섹션 3이 당분간 그 역할.
  에이전트 프롬프트가 커져 분리가 필요한 시점에 생성.

---

## 10. 다음 작업 우선순위 (2026-04-22 기준)

**오늘 실행**

1. **Zod 도입** — 5개 에이전트 전체. 마이그레이션 순서: Self Insight(`---SAVE---` 마커가 가장 취약) → Problem Scout → Validation Designer → Fit Judge → Reality Check.
2. Validation Backlog 이용 Flow 끊김 - 문제 선택 후 '검증 플랜'버튼 누르면 로딩 후 아무 결과 없음. 오류 메세지도 없음.
3. **`LearningLog` 엔티티 + 역류 트랜잭션** — Prisma 스키마 → 마이그레이션 → `lib/db/learning-log.ts`에 트랜잭션 구현 → route 연결.

**그 다음**

3. **`ideaDraft` → `solutionHypothesis` 리네이밍 패키지** (스키마 + 프롬프트 + UI 카피).
4. **Problem Scout URL 검증** — 저장 전 HEAD 요청 또는 web_fetch.
5. **Reality Check 반증 질문 프롬프트 튜닝**.
6. **`timeboxWeeks` · `sourceVerifiedAt` · `sourceMessageId` 스키마 추가** — 리네이밍 마이그레이션과 묶어 한 번에.
7. **`lib/agents/` + `lib/db/` 구조 이동** — 위 작업들로 프롬프트와 쿼리가 한 번 흔들린 뒤에 박제.

**언제든**

8. 시드 하드코딩 → JSON 파일 이동.
9. Fit Judge `reason` 필드에 Self map 엔트리 ID 참조 구조.
